...
SPANNER <- function(start,vector_1,vector_2){
  S_1 <- as.vector(sapply(
    start,
    function(x) which(cumsum(vector_1[x:length(vector_2)]) >= stage_1)[1]))
  S_2 <- as.vector(sapply(
    start + S_1,
    function(x) which(cumsum(vector_2[x:length(vector_2)]) >= stage_2)[1]))
  S_3 <- as.vector(sapply(
    start + S_1 + S_2,
    function(x) which(cumsum(vector_1[x:length(vector_2)]) >= stage_3)[1]))
  S_4 <- as.vector(sapply(
    start + S_1 + S_2 + S_3,
    function(x) which(cumsum(vector_1[x:length(vector_2)]) >= stage_4)[1]))
  S_5 <- as.vector(sapply(
    start + S_1 + S_2 + S_3 + S_4,
    function(x) which(cumsum(vector_1[x:length(vector_2)]) >= stage_5)[1]))
  S_6 <- as.vector(sapply(
    start + S_1 + S_2 + S_3 + S_4 + S_5,
    function(x) which(cumsum(vector_1[x:length(vector_2)]) >= stage_6)[1]))
  S_7 <- as.vector(sapply(
    start + S_1 + S_2 + S_3 + S_4 + S_5 + S_6,
    function(x) which(cumsum(vector_1[x:length(vector_2)]) >= stage_7)[1]))
