...
For a given point location I can calculate the average temperature or relative humidity during flowering for a specific planting date and compare the values under climate change to the historical baseline. If relative humidity and temperature during flowering are forecasted to increase under climage change this may suggest increased FHB risk in the future. The nice part about leaving all the parameters out in the open is that we can look into how changes in the input parameters may mitigate FHB risk and better prepare growers. 
